...
class ActionFlyBy(Action):
    ...
    def __init__(
        ...
        if self.reverse:
            ...
            self.final_pos = mm2steps(
                measrng.max() + s_ru, self.stage.convunits["pos"]
            )
            self.data = {
...
class ScanRoutine():
    ...
    def build(self):
        ...
        if self.line_type == LineType.FLYBY:
            ...
            # do a single line
            reverse = False
            self.actions.append(ActionFlyBy(
            ...
            self.ta += calc_movetime(
                ...
                dist=2*s_ru[self.order[0].value]+self.ranges[self.order[0].value].max()-self.ranges[self.order[0].value].min()
            )
            ...
            if len(self.ranges[self.order[1].value]) > 1 or len(self.ranges[self.order[2].value]) > 1:
                    ...
                    for j in range_1:
                        ...
                        if self.line_start == LineStart.SNAKE:
                            if reverse:
                                ...
                                new_line_pos[self.order[0].value] = self.ranges[self.order[0].value].min() - s_ru[self.order[0].value]
                                pass
                            else:
                                # stage is at max of order[0] range
                                new_line_pos[self.order[0].value] = self.ranges[self.order[0].value].max() + s_ru[self.order[0].value]
                        ...
                        self.ta += calc_movetime(
                            stage=self.stages[self.order[0].value],
                            dist=2*s_ru+self.ranges[self.order[0].value].max()-self.ranges[self.order[0].value].min()
                        )
                ...
                for i in range_2:
                    for j in range_1:
                        ...
                        if self.line_start == LineStart.SNAKE:
                            if reverse:
                                # stage is at min of order[0] range
                                new_line_pos[self.order[0].value] = self.ranges[self.order[0].value].min()
                                pass
                            else:
                                # stage is at max of order[0] range
                                new_line_pos[self.order[0].value] = self.ranges[self.order[0].value].max()
